...
function mattata:process_plugin_extras()
    ...
    then
        mattata.send_reply(
            message,
            'Ye kya command h?? use /help to get the commands I work on!'
        )
    end
    return true
end
...
function mattata:on_inline_query()
    ...
    elseif redis:get('global_blacklist:' .. inline_query.from.id) then
        return false, 'This user is globally blacklisted(Globally matlab globally)!'
    end
...
function mattata.format_time(seconds)
    ...
    elseif hours < 24 then
        return hours ~= 1 and hours .. ' Gantey' or hours .. ' Ganta'
    end
    local days = math.floor(seconds / 86400)
    if days == 0 then
        return hours ~= 1 and hours .. ' Gantey' or hours .. ' Ganta'
    elseif days < 7 then
        return days ~= 1 and days .. ' din' or days .. ' din'
    end
    local weeks = math.floor(seconds / 604800)
    if weeks == 0 then
        return days ~= 1 and days .. ' din' or days .. ' din'
    else
        return weeks ~= 1 and weeks .. ' hafte' or weeks .. ' hafte'
    end
end
...
function mattata.process_spam(message)
    ...
    -- Don't run the antispam plugin if the user is configured as a global admin in `configuration.lua`.
        ...
        -- too many messages.
            message,
            string.format(
                'Hey %s, Itne Msgs mat bhejo, nhi to muje use nhi kar paoge...pyaar se samja rha hu samj ja',
    ...
    then
        ...
        mattata.send_reply(
            message,
            string.format(
                'Hey, %s,tuje ek bar bola samj me nhi aya to maine tuje blacklist kardiya ab tu muje use nhi kar paega 24hrs tak!',
                message.from.username and '@' .. message.from.username or message.from.name
            )
        )
        return true
...
function mattata.process_deeplinks(message)
    if message.text:match('^/%-%d+%_%a+$') and message.chat.type == 'private' then
        ...
        if action == 'rules' and mattata.get_setting(chat_id, 'use administration') then
            local rules = mattata.get_value(chat_id, 'rules') or 'Rules ki setting adhuri h!'
...
function mattata.check_links(message, get_links, only_valid, whitelist)
    ...
    if whitelist then
        ...
        return string.format(
            '%s link%s ha%s ko whitelist kiya gaya!',
            count,
            count == 1 and '' or 's',
            count == 1 and 's' or 've'
        )
...
function mattata:process_message()
    ...
    if message.new_chat_members and mattata.get_setting(message.chat.id, 'use administration') and mattata.get_setting(message.chat.id, 'antibot') and not mattata.is_group_admin(message.chat.id, message.from.id) and not mattata.is_global_admin(message.from.id) then
        ...
        if #kicked > 0 and #usernames > 0 and #kicked == #usernames then
            ...
            mattata.send_message(log_chat, string.format('<pre>%s [%s] Ko Nikala gaya %s , %s [%s] group se kyunki anti-bot enabled h.</pre>', mattata.escape_html(self.info.first_name), self.info.id, table.concat(kicked, ', '), mattata.escape_html(message.chat.title), message.chat.id), 'html')
            return mattata.send_message(message, string.format('Kicked %s kyunki anti-bot enabled tha.', table.concat(usernames, ', ')))
        end
    end
        ...
        if words and #words > 0 then
            for k, v in pairs(words) do
                ...
                if text:match('^' .. v:lower() .. '$') or text:match('^' .. v:lower() .. ' ') or text:match(' ' .. v:lower() .. ' ') or text:match(' ' .. v:lower() .. '$') then
                    mattata.delete_message(message.chat.id, message.message_id)
                    local action = mattata.get_setting(message.chat.id, 'ban,kick nhi') and mattata.ban_chat_member or mattata.kick_chat_member
                    ...
                    if success then
                        if mattata.get_setting(message.chat.id, 'log administrative actions') then
                            local log_chat = mattata.get_log_chat(message.chat.id)
                            mattata.send_message(log_chat, string.format('<pre>%s [%s] ko nikala gaya %s [%s] from %s [%s] group se prohibited words istemal karne ke liye.</pre>', mattata.escape_html(self.info.first_name), self.info.id, mattata.escape_html(message.from.first_name), message.from.id, mattata.escape_html(message.chat.title), message.chat.id), 'html')
                        end
                        mattata.send_message(message.chat.id, string.format('Kicked %s Prohibited word istemal karne ke liye.', message.from.username and '@' .. message.from.username or message.from.first_name))
                        break_cycle = true
                    end
                end
            end
    ...
    if message.chat.type == 'supergroup' and mattata.get_setting(message.chat.id, 'use administration') and mattata.get_setting(message.chat.id, 'antilink') and not mattata.is_group_admin(message.chat.id, message.from.id) and not mattata.is_global_admin(message.from.id) and mattata.check_links(message) then
        ...
        if success then
            if mattata.get_setting(message.chat.id, 'log administrative actions') then
                local log_chat = mattata.get_log_chat(message.chat.id)
                mattata.send_message(log_chat, string.format('<pre>%s [%s] ko nikala gaya %s [%s] from %s [%s] group se for sending Telegram invite link(s)</pre>', mattata.escape_html(self.info.first_name), self.info.id, mattata.escape_html(message.from.first_name), message.from.id, mattata.escape_html(message.chat.title), message.chat.id), 'html')
            end
            return mattata.send_message(message.chat.id, string.format('nikala gaya %s for sending Telegram invite link(s).', message.from.username and '@' .. message.from.username or message.from.first_name))
        end
    end
    if message.new_chat_members and message.chat.type ~= 'private' and mattata.get_setting(message.chat.id, 'use administration') and mattata.get_setting(message.chat.id, 'welcome message') then
        ...
        local keyboard
        if mattata.get_setting(message.chat.id, 'rules bheje join karne pe') then
